...
const initialState: HomeServerRootState = {
  ...
  seedboxTodoSubState: {
    ...
    currentTargetDirectory:
      FileDirectoryDescriptor.emptyFileDirectoryDescriptor(),
  ...
  musicPlayerSubState: {
    musicPlayerStatus: MusicPlayerStatus.empty(),
  },
};

const reducer = (
  previousState: HomeServerRootState,
...
const HomeServerProvider: FC<HomeServerProviderProps> = (props) => {
  ...
  return (
    <HomeServerRootContext.Provider value={{ ...state, dispatch }}>
      {props.children}
    </HomeServerRootContext.Provider>
  );
};

const useHomeServerContext: () => HomeServerRootState = () =>
  useContext(HomeServerRootContext);

export { HomeServerProvider, useHomeServerContext };
