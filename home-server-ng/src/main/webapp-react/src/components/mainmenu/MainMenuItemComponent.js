...
export default function MainMenuItemComponent({ menuDescriptor, currentModule, onItemClick }) {
    ...
    const innerItemClick = selectedElement => {
        eventReactor().emit(OTHER_MAIN_MENU_ITEM_CLICK);
    ...
    const menuItem = menuDescriptor.subMenu !== undefined ?
        ...
        :
        <a href="#!" onClick={() => innerItemClick(menuDescriptor)}><i className="material-icons left">{menuDescriptor.icon}</i>{menuDescriptor.title}</a>
        ;
            ...
            <ul id={dropDownDescription.targetId} className="dropdown-content" style={subMenuStatus.style}>
                {
                    menuDescriptor?.subMenu?.map((oneSubMenuItem) =>
                        <li key={"main_menu_item_sub_menu_" + oneSubMenuItem.title} onClick={() => innerItemClick(oneSubMenuItem)}>
                            <a href="#!">
